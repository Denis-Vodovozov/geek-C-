...
void FindElemOfArray(int[,] array, int rowind, int columnind)
{
    if (rowind >= 0 && rowind < array.GetLength(0) && columnind >= 0 && columnind < array.GetLength(1))
        Console.WriteLine($"Элемент на позиции ({rowind}; {columnind}) это {array[rowind, columnind]}");
    else Console.WriteLine($"Элемента на позиции ({rowind}; {columnind}) нет");
}
Console.Write("Введите количество строк: ");
int m = Convert.ToInt32(Console.ReadLine());
Console.Write("Введите количество столбцов: ");
...
Show2dArray(myArray);
FindElemOfArray(myArray, rownum, columnnum);
*/
...
double[] AverageElemColumns(int[,] array)
{
...
int[,] myArray = CreateRandom2dArray(m, n, min, max);
Show2dArray(myArray);
double[] averageColumns = AverageElemColumns(myArray);
Console.WriteLine("Среднее арифметическое элементов в каждом столбце:");
ShowArray(averageColumns);
*/